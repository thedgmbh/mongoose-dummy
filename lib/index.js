...
let getPaths = (model) => {
    ...
    let _generateReturnObj = (path, name, schema, context) => { /* Generate definition for a single path */
        ...
        if(path.instance.toLowerCase() === "embedded"){
            ...
            if (!path.schema) {
                result.definition = _generateReturnObj(path.caster);
            } else {
                result.definition = getPaths(path.schema);
            }
        }
    ...
    for (let field in paths) { /* Loop over paths and generated data */
        ...
        if (desc.isEnum) { /* Handle enumerations paths */
            ...
            if (type === "string") {
                ...
                if (desc.ref) {
                    generated[field] = generateRandomModel({generate: desc.definition}, opts).generate; /* Handle arrays with primitives */
                } else {
                    generated[field] = generateRandomModel(desc.definition, opts); /* Handle arrays with defined objects */
                }
