 ...
 *
 * Generate random object from paths
 *
 ...
 */
let generateRandomModel = async (paths, opts) => {
  let generated = {};
  ...
  for (let field in paths) { /* Loop over paths and generated data */
    ...
    if (desc.isEnum) { /* Handle enumerations paths */
      ...
      if (type === "string") {
        ...
        if (desc.ref) {
          let newOpts = merge(cloneDeep(opts), {custom: opts.custom[field] || {}});
          generated[field] = await generateNestedModel(desc.ref, newOpts);
        } else {
...
let generateNestedModel = async (modelName, opts) => {
  try {
    ...
    let obj = await generateRandomModel(paths, opts);
    ...
    console.warn(`Unable to generate nested model: ${error.message}`);
